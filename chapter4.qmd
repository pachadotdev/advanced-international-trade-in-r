...
if (!file.exists(fout)) {
  bbg96_87 <- fread(finp)
  setnames(bbg96_87, tolower(names(bbg96_87)))
  bbg96_87[, year := year + 1900]
  bbg96_87 <- bbg96_87[year >= 1958 & year <= 1996]
...
wage_rates[, npwwr_pwwr := npwwr / pwwr]

# Compute the relative nonproduction/production employment ----

employment_rates <- bbg96_87[, .(
  pemp = sum(prode),
  npemp = sum(emp) - sum(prode)
), by = year]
employment_rates[, npemp_pemp := npemp / pemp]
...
tinyplot(
  ...
  data = wage_rates,
  type = "b",
  pch = 19,# Figure 4.1: Relative wage of nonproduction / production workers ----
...
tinyplot(
  ...
  pch = 19,
  cex = 1.5,
  main = "Relative wage of nonproduction / production workers, U.S. Manufacturing",
  xlab = "Year",
  ylab = "Relative wage"
)
...
tinyplot(
  ...
  pch = 19,
  cex = 1.5,
  main = "Relative employment of nonproduction / production workers, U.S. Manufacturing",
  xlab = "Year",
  ylab = "Relative employment"
)
  cex = 1.5,
  main = "Relative wage of nonproduction / production workers, U.S. Manufacturing",
  xlab = "Year",
...
if (!file.exists(fout)) {
  ...
  datachp4 <- datachp4[!year %in% c(1972, 1987)]
  datachp4 <- datachp4[!sic72 %in% c(2067, 2794, 3483)]
  datachp4[, wagebill := sum(pay), by = year]
  datachp4[, share := pay / wagebill]
  setorder(datachp4, sic72, year)
  datachp4[, lagshare := shift(share), by = sic72]
  datachp4[, ashare := (share + lagshare) / 2]
  datachp4[, lagnwsh := shift(nwsh), by = sic72]
  datachp4[, chanwsh := (nwsh - lagnwsh) * 100 / 11]
  datachp4[, wchanwsh := chanwsh * ashare]
  datachp4[, wdlky := dlky * ashare]
  datachp4[, wdly := dly * ashare]
  datachp4[, wdsimat1a := dsimat1a * ashare]
  datachp4[, wdsimat1b := dsimat1a * ashare]
  datachp4[, diffout := dsimat1a - dsimat1b]
  datachp4[, wdiffout := (dsimat1a - dsimat1b) * ashare]
  datachp4[, wcosh_exp := dofsh * ashare]
  datachp4[, htsh_exp := dhtsh - dofsh]
  datachp4[, whtsh_exp := (dhtsh - dofsh) * ashare]
  datachp4[, wcosh_exa := dofsh1 * ashare]
  datachp4[, htsh_exa := dhtsh1 - dofsh1]
  datachp4[, whtsh_exa := (dhtsh1 - dofsh1) * ashare]
  datachp4[, wcosh := ci * ashare]
  datachp4[, whtsh := dhtsh * ashare]
}

# Check with the first column of Table 4.4 ----

sum_cols <- names(datachp4)[
  which(names(datachp4) == "wchanwsh"):which(names(datachp4) == "whtsh")
]
datachp4[, lapply(.SD, sum, na.rm = TRUE), .SDcols = sum_cols]
...
datachp4 <- readRDS(fout)
datachp4 <- datachp4[year == 1990]
datachp4 <- datachp4[!sic72 %in% c(2067, 2794, 3483)]
datachp4[, etfp := ptfp - err]
datachp4[, adj1 := 1 / (1 - amesh)]
datachp4[, etfp1 := adj1 * etfp]
datachp4[, dlpvad1 := adj1 * dlpvad]
datachp4[, apsh1 := adj1 * apsh]
datachp4[, ansh1 := adj1 * ansh]
datachp4[, aksh1 := adj1 * aksh]
datachp4[, mshxpr := amsh * dlpmx]
datachp4[, eshxpr := aosh * dlpe]

# Reproduce Table 4.5 ----

datachp4[, dlp34 := dlp - mshxpr - eshxpr]
...
datachp4_2 <- datachp4[sic72 != 3573]
...
datachp4[, ansh1 := adj1 * ansh]
datachp4[, aksh1 := adj1 * aksh]
datachp4[, t4dlpvad := (dlpvad + etfp) * adj1]

# Reproduce the first column of Table IV ----

# generating difference measure of outsourcing
datachp4_2 <- copy(datachp4)
datachp4_2[, dsimatd1 := dsimat1a - dsimat1b]

# generating difference measure of high tech share
datachp4_2[, dhtdsh := dhtsh - dofsh]
...
long <- melt(datachp4_2, id.vars = "mvshipsh",
             measure.vars = c("dsimatd1", "dhtdsh", "dofsh"),
             variable.name = "var", value.name = "val")
stats_table <- long[, .(
  nobs = .N,
  wsum = sum(val * mvshipsh),
  weighted_sd = weighted.sd(val, mvshipsh),
  min = min(val),
  max = max(val)
), by = var]
sum_weights <- sum(datachp4_2$mvshipsh)
stats_table
sum_weights
...
datachp4_2[, wt := sqrt(mvshipsh)]
datachp4_2[, apsh5 := apsh1 * wt]
datachp4_2[, ansh5 := ansh1 * wt]
datachp4_2[, aksh5 := aksh1 * wt]
datachp4_2[, narrout := dsimat1b * wt * coef(reg1)["dsimat1b"]]
datachp4_2[, diffout := dsimatd1 * wt * coef(reg1)["dsimatd1"]]
datachp4_2[, comsh := dofsh * wt * coef(reg1)["dofsh"]]
datachp4_2[, difcom := dhtdsh * wt * coef(reg1)["dhtdsh"]]

long2 <- melt(datachp4_2, measure.vars = c("narrout", "diffout", "comsh", "difcom"),
              variable.name = "var", value.name = "val")
long2[, .(
  nobs = .N,
  mean = mean(val),
...
## generating difference measure of outsourcing
datachp4[, dsimatd1 := dsimat1a - dsimat1b]

## generate difference measure of high tech share with ex ante rental price
datachp4[, dhtdsh1 := dhtsh1 - dofsh1]

## check whether we are using the right variable as described in table II

long3 <- melt(datachp4, id.vars = "mvshipsh",
              measure.vars = c("dsimatd1", "dhtdsh1", "dofsh1"),
              variable.name = "var", value.name = "val")
stats_table2 <- long3[, .(
  nobs = .N,
  wsum = sum(val * mvshipsh),
...
sum_weights <- sum(datachp4$mvshipsh)
stats_table2
sum_weights
