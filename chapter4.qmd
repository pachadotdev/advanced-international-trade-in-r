...
tinyplot(
  ...
  sub = "Source: NBER productivity database (Bartelsman and Gray 1996)",
  xlab = "Year",
  ylab = "Relative wage"
)
...
tinyplot(
  ...
  type = "b",    subtitle = "Source: NBER productivity database (Bartelsman and Gray 1996)",
  pch = 19,
...
if (!file.exists(fout)) {
  datachp4 <- as.data.table(read_dta(paste0(dout, "/data_Chp4.dta")))
  saveRDS(datachp4, fout)
}

datachp4 <- readRDS(fout)
datachp4 <- datachp4[!year %in% c(1972, 1987) & !sic72 %in% c(2067, 2794, 3483)]

datachp4[, share := pay / sum(pay), by = year]

setorder(datachp4, sic72, year)
datachp4[, ashare := (share + shift(share)) / 2, by = sic72]
datachp4[, chanwsh := (nwsh - shift(nwsh)) * 100 / 11, by = sic72]
...
## generating difference measure of outsourcing and high tech share with ex ante rental price
datachp4[, `:=`(
  dsimatd1 = dsimat1a - dsimat1b,
  dhtdsh1  = dhtsh1 - dofsh1
)]
