...
if (!file.exists(fout)) {
  feenstra_88 <- list(
    car_7990 = as.data.table(read_dta(paste0(dout, "/car_7990.dta"))),
    truck_7990 = as.data.table(read_dta(paste0(dout, "/truck_7990.dta")))
  )
...
car_7990 <- feenstra_88$car_7990[, .(year, model, type, price, quan)]
setorder(car_7990, year, model)

car_80 <- car_7990[year == 80, .(model, type, price_80 = price, quan_80 = quan)]

car_7990 <- merge(car_7990, car_80, by = c("model", "type"))

car_7990[, value_80 := sum(price_80 * quan_80), by = year]
car_7990[, value_cp := sum(price * quan_80), by = year]
car_7990[, lasp := value_cp / value_80]

car_7990[, value_c := sum(price * quan), by = year]
car_7990[, value_cq := sum(price_80 * quan), by = year]
car_7990[, pasp := value_c / value_cq]

car_7990[, pindex := sqrt(lasp * pasp)]
car_7990[, .(pindex = mean(pindex)), by = year]
```

#### pindex_t.do

```{r ch8_ex1b}
truck_7990 <- feenstra_88$truck_7990[, .(year, model, type, price, quan)]
setorder(truck_7990, year, model)

truck_80 <- truck_7990[year == 80, .(model, type, price_80 = price, quan_80 = quan)]

truck_7990 <- merge(truck_7990, truck_80, by = c("model", "type"))

truck_7990[, value_80 := sum(price_80 * quan_80), by = year]
truck_7990[, value_cp := sum(price * quan_80), by = year]
truck_7990[, lasp := value_cp / value_80]

truck_7990[, value_c := sum(price * quan), by = year]
truck_7990[, value_cq := sum(price_80 * quan), by = year]
truck_7990[, pasp := value_c / value_cq]

truck_7990[, pindex := sqrt(lasp * pasp)]
truck_7990[, .(pindex = mean(pindex)), by = year]
```

#### unit_value.do

```{r ch8_ex1c}
car_7990 <- copy(feenstra_88$car_7990)
car_7990[, value := price * quan]
car_7990[, aquan := mean(quan), by = year]
car_7990[, avalue := mean(value), by = year]
car_7990[, uvalue := avalue / aquan]

truck_7990 <- copy(feenstra_88$truck_7990)
truck_7990[, value := price * quan]
truck_7990[, aquan := mean(quan), by = year]
truck_7990[, avalue := mean(value), by = year]
truck_7990[, uvalue := avalue / aquan]

car_7990[, .(uvalue = mean(uvalue)), by = year]
truck_7990[, .(uvalue = mean(uvalue)), by = year]
```
...
car_7990 <- feenstra_88$car_7990[year < 86]
car_7990[, yd := factor(year)]
car_7990[, id := seq_len(.N), by = yd]

tab <- car_7990[, .(freq = .N), by = yd]
tab[, percent := freq / sum(freq) * 100]
tab[, cum := cumsum(percent)]
tab

# expand the yd variable to 0/1 columns
# in R, you cannot multiply a factor/categorical variable

car_7990[, yd_ind := as.factor(as.integer(yd) - min(as.integer(yd)) + 1)]

car_7990_dummy <- model.matrix(~ yd_ind - 1, data = car_7990)
colnames(car_7990_dummy) <- gsub("_ind", "", colnames(car_7990_dummy))
car_7990_dummy <- as.data.table(car_7990_dummy)
car_7990[, names(car_7990_dummy) := car_7990_dummy]
...
car_7990[, winv := 1 / exp(fit_coef["b2"] * yd2 + fit_coef["b3"] * yd3 +
...
car_7990[, nprice := price * winv]
car_7990[, nyd3 := yd3 * winv]
car_7990[, nyd4 := yd4 * winv]
car_7990[, nyd5 := yd5 * winv]
car_7990[, nyd6 := yd6 * winv]
car_7990[, nyd7 := yd7 * winv]
...
```{r ch8_ex2b}
truck_7990 <- feenstra_88$truck_7990[year < 86]
setnames(truck_7990, "weight", "wght")
truck_7990[, lnprice := log(price)]
truck_7990[, yd := factor(year)]
truck_7990[, id := seq_len(.N), by = yd]

tab <- truck_7990[, .(freq = .N), by = yd]
tab[, percent := freq / sum(freq) * 100]
tab[, cum := cumsum(percent)]
tab
...
truck_7990[, yd_ind := as.factor(as.integer(yd) - min(as.integer(yd)) + 1)]
...
truck_7990[, winv := 1 / exp(fit_coef["a2"] * yd2 + fit_coef["a3"] * yd3 +
...
reshape_estimates <- function(fit, vehicle, term_prefix, term_mappings) {
  ...
  setnames(coef_dt, c("term", "estimate", "std.error", "statistic", "p.value"))
  coef_dt <- coef_dt[term %in% paste0(term_prefix, 1:8), .(term, estimate, std.error)]
  for (i in seq_along(term_mappings)) {
    coef_dt[term == paste0(term_prefix, i), term := term_mappings[i]]
  }
  coef_dt <- melt(coef_dt, id.vars = "term", measure.vars = c("estimate", "std.error"),
                  variable.name = "stat", value.name = vehicle)
  coef_dt[stat == "estimate", stat := "Estimate"]
  coef_dt[stat == "std.error", stat := "Std. Error"]
  coef_dt[, (vehicle) := round(get(vehicle), 3)]
  coef_dt
}
...
table_84_1 <- merge(
  reshape_estimates(fit2, "Car", "c", car_vars),
  reshape_estimates(fit4, "Truck", "b", truck_vars),
  by = c("term", "stat"),
  all = TRUE
)
# keep Constant at the beginning
table_84_1[, term := factor(term, levels = ordered_vars)]
setorder(table_84_1, term)

setnames(table_84_1, c("Variable", "Statistic", "Car", "Truck"))
...
car_7990 <- feenstra_88$car_7990[year < 86]
car_7990[, ve := 1]
setnames(car_7990, c("wdth", "hght", "wght", "hp", "four", "tran", "ps", "ac"),
         c("wdth_c", "hght_c", "wght_c", "hp_c", "four_c", "tran_c", "ps_c", "ac_c"))

truck_7990 <- feenstra_88$truck_7990[year < 86]
truck_7990[, ve := 2]
setnames(truck_7990, c("wdth", "hght", "weight", "hp", "four", "tran", "ps", "ac"),
         c("wdth_t", "hght_t", "wght_t", "hp_t", "four_t", "tran_t", "ps_t", "ac_t"))

truck_7990 <- rbindlist(list(truck_7990, car_7990), fill = TRUE)

na_cols <- c("wdth_c", "hght_c", "wght_c", "hp_c", "four_c", "tran_c", "ps_c", "ac_c",
             "wdth_t", "hght_t", "wght_t", "hp_t", "four_t", "tran_t", "ps_t", "ac_t")
for (col in na_cols) {
  truck_7990[is.na(get(col)), (col) := 0]
}

tab <- truck_7990[, .(freq = .N), by = year]
tab[, percent := freq / sum(freq) * 100]
tab[, cum := cumsum(percent)]
tab
...
truck_7990[, cyd2 := fifelse(type == "JT", 0, yd2)]
truck_7990[, cyd3 := fifelse(type == "JT", 0, yd3)]
truck_7990[, cyd4 := fifelse(type == "JT", 0, yd4)]
truck_7990[, cyd5 := fifelse(type == "JT", 0, yd5)]
truck_7990[, cyd6 := fifelse(type == "JT", 0, yd6)]
truck_7990[, cyd7 := fifelse(type == "JT", 0, yd7)]
truck_7990[, tyd2 := fifelse(type != "JT", 0, yd2)]
truck_7990[, tyd3 := fifelse(type != "JT", 0, yd3)]
truck_7990[, tyd4 := fifelse(type != "JT", 0, yd4)]
truck_7990[, tyd5 := fifelse(type != "JT", 0, yd5)]
truck_7990[, tyd6 := fifelse(type != "JT", 0, yd6)]
truck_7990[, tyd7 := fifelse(type != "JT", 0, yd7)]
...
truck_7990[ve == 1, winv := 1 / exp(coef(fit)["b2"] * cyd2 + coef(fit)["b3"] * cyd3 +
...
truck_7990[ve == 2, winv := 1 / exp((coef(fit)["b2"] + 0.16) * tyd2 +
...
```{r ch8_ex3_table}
coef_dt <- as.data.table(summary(fit2)$coefficients, keep.rownames = "term")
setnames(coef_dt, c("term", "estimate", "std.error", "statistic", "p.value"))

coef_dt <- coef_dt[, .(term, estimate, std.error)]

coef_dt[, vehicle := fcase(
  substr(term, 1, 1) == "c", "car",
  substr(term, 1, 1) == "d", "truck",
  default = "Exclude"
)]
coef_dt <- coef_dt[vehicle != "Exclude"]

coef_dt[, term := fcase(
  term %in% c("c1", "d1"), "Weight (tons)",
  term %in% c("c2", "d2"), "Width (feet)",
  term %in% c("c3", "d3"), "Height (feet)",
  term %in% c("c4", "d4"), "Horsepower (100)",
  term %in% c("c5", "d5"), "Transmission (5-speed or auto)",
  term %in% c("c6", "d6"), "Power steering",
  term == "c7",            "Air conditioning",
  term == "d7",            "Four-wheel drive",
  term %in% c("c8", "d8"), "Constant",
  default = "Exclude"
)]
coef_dt <- coef_dt[term != "Exclude"]

table_84_2 <- melt(coef_dt, id.vars = c("term", "vehicle"),
                   measure.vars = c("estimate", "std.error"),
                   variable.name = "stat", value.name = "value")
table_84_2 <- dcast(table_84_2, term + stat ~ vehicle, value.var = "value")

num_cols <- names(table_84_2)[sapply(table_84_2, is.numeric)]
table_84_2[, (num_cols) := lapply(.SD, round, 3), .SDcols = num_cols]
...
table_84_2[, term := factor(term, levels = term_levels)]
table_84_2[stat == "estimate", stat := "Estimate"]
table_84_2[stat == "std.error", stat := "Std. Error"]
setorder(table_84_2, term)

setnames(table_84_2, c("Variable", "Statistic", "Car", "Truck"))
...
truck_7990[, ve := 2]
setnames(truck_7990,
         c("quan", "wdth", "hght", "weight", "hp", "four", "tran", "ps", "ac"),
         c("quan_t", "wdth_t", "hght_t", "wght_t", "hp_t", "four_t", "tran_t", "ps_t", "ac_t"))

car_7990 <- feenstra_88$car_7990[year < 86]
car_7990[, ve := 1]
setnames(car_7990,
         c("quan", "wdth", "hght", "wght", "hp", "four", "tran", "ps", "ac"),
         c("quan_c", "wdth_c", "hght_c", "wght_c", "hp_c", "four_c", "tran_c", "ps_c", "ac_c"))
...
truck_7990[ve == 1, winv := 1 / exp(fit_coef["b2"] * cyd2 + fit_coef["b3"] * cyd3 +
...
truck_7990[ve == 2, winv := 1 / exp((fit_coef["b2"] + 0.16) * tyd2 +
...
```{r ch8_ex4b}
truck_7990_2 <- truck_7990[ve == 1]
truck_7990_2[, quality := exp(fit2_coef["c8"] + fit2_coef["b2"] +
    fit2_coef["c1"] * wght_c + fit2_coef["c2"] * wdth_c +
    fit2_coef["c3"] * hght_c + fit2_coef["c4"] * hp_c +
    fit2_coef["c5"] * tran_c + fit2_coef["c6"] * ps_c + fit2_coef["c7"] * ac_c)]
truck_7990_2[, nvalue := quality * quan_c]

car_uquality <- truck_7990_2[, .(aquan = mean(quan_c), anvalue = mean(nvalue)), by = year]
car_uquality[, mean_uquality := anvalue / aquan]
car_uquality <- car_uquality[, .(year, mean_uquality)]
car_uquality

truck_7990_3 <- truck_7990[ve == 2]
truck_7990_3[, quality := exp(fit2_coef["d8"] + fit2_coef["b2"] +
    fit2_coef["d1"] * wght_t + fit2_coef["d2"] * wdth_t +
    fit2_coef["d3"] * hght_t + fit2_coef["d4"] * hp_t +
    fit2_coef["d5"] * tran_t + fit2_coef["d6"] * ps_t + fit2_coef["d7"] * four_t)]
truck_7990_3[, nvalue := quality * quan_t]

truck_uquality <- truck_7990_3[, .(aquan = mean(quan_t), anvalue = mean(nvalue)), by = year]
truck_uquality[, mean_uquality := anvalue / aquan]
truck_uquality <- truck_uquality[, .(year, mean_uquality)]
truck_uquality
```
...
```{r ch8_ex4c}
car_7990_2_80 <- truck_7990_2[year == 80, .(model, type, quality_80 = quality, quan_80 = quan_c)]

car_qindex <- truck_7990_2[, .(year, model, type, quality, quan_c)]
car_qindex <- merge(car_qindex, car_7990_2_80, by = c("model", "type"))

car_qindex[, value_80 := sum(quality_80 * quan_80), by = .(year, model)]
car_qindex[, value_cp := sum(quality * quan_80), by = .(year, model)]
car_qindex[, lasp := value_cp / value_80]

car_qindex[, value_c := sum(quality * quan_c), by = year]
car_qindex[, value_cq := sum(quality_80 * quan_c), by = year]
car_qindex[, pasp := value_c / value_cq]
car_qindex[, qindex := sqrt(lasp * pasp)]

car_qindex <- car_qindex[, .(mean_qindex = mean(qindex)), by = year]
car_qindex
```
...
```{r ch8_ex4d}
truck_7990_3_80 <- truck_7990_3[year == 80, .(model, type, quality_80 = quality, quan_80 = quan_t)]

truck_qindex <- truck_7990_3[, .(year, model, type, quality, quan_t)]
truck_qindex <- merge(truck_qindex, truck_7990_3_80, by = c("model", "type"))

truck_qindex[, value_80 := sum(quality_80 * quan_80), by = .(year, model)]
truck_qindex[, value_tp := sum(quality * quan_80), by = .(year, model)]
truck_qindex[, lasp := value_tp / value_80]

truck_qindex[, value_t := sum(quality * quan_t), by = year]
truck_qindex[, value_tq := sum(quality_80 * quan_t), by = year]
truck_qindex[, pasp := value_t / value_tq]
truck_qindex[, qindex := sqrt(lasp * pasp)]

truck_qindex <- truck_qindex[, .(mean_qindex = mean(qindex)), by = year]
truck_qindex
```
...
reshape_qindex <- function(data, mean_qindex_col, group, measurement) {
  dt <- copy(data)
  setnames(dt, mean_qindex_col, "mean_qindex")
  setorder(dt, year)
  dt[, qindex_pct := (mean_qindex - shift(mean_qindex)) / mean_qindex]

  dt_long <- melt(dt, id.vars = "year",
                  measure.vars = c("mean_qindex", "qindex_pct"),
                  variable.name = "metric", value.name = "value")
  dt_wide <- dcast(dt_long, metric ~ year, value.var = "value")

  dt_wide[, type := group]
  dt_wide[, metric := c(measurement, paste(measurement, "(% Change)"))]

  year_cols <- setdiff(names(dt_wide), c("type", "metric"))
  setcolorder(dt_wide, c("type", "metric", year_cols))

  num_cols <- names(dt_wide)[sapply(dt_wide, is.numeric)]
  dt_wide[, (num_cols) := lapply(.SD, round, 3), .SDcols = num_cols]

  dt_wide
}

table83 <- rbindlist(list(
  reshape_qindex(car_uquality, "mean_uquality", "Japanese Cars", "Unit quality"),
...
truck_7990[, cyd5  := fifelse(type == "JT", 0,  yd5)]
truck_7990[, cyd6  := fifelse(type == "JT", 0,  yd6)]
truck_7990[, cyd7  := fifelse(type == "JT", 12, yd7)]
truck_7990[, cyd8  := fifelse(type == "JT", 0,  yd8)]
truck_7990[, cyd9  := fifelse(type == "JT", 0,  yd9)]
truck_7990[, cyd10 := yd10]
truck_7990[, cyd11 := yd11]
truck_7990[, cyd12 := yd12]
truck_7990[, tyd2  := fifelse(type != "JT", 0,  yd2)]
truck_7990[, tyd3  := fifelse(type != "JT", 0,  yd3)]
truck_7990[, tyd4  := fifelse(type != "JT", 0,  yd4)]
truck_7990[, tyd5  := fifelse(type != "JT", 0,  yd5)]
truck_7990[, tyd6  := fifelse(type != "JT", 0,  yd6)]
truck_7990[, tyd7  := fifelse(type != "JT", 12, yd7)]
truck_7990[, tyd8  := fifelse(type != "JT", 0,  yd8)]
truck_7990[, tyd9  := fifelse(type != "JT", 0,  yd9)]
truck_7990[, tyd10 := yd10]
truck_7990[, tyd11 := yd11]
truck_7990[, tyd12 := yd12]
```
...
truck_7990[ve == 2, winv := 1 / exp(fit_coef["e2"] * tyd2 + fit_coef["e3"] * tyd3 +
...
t1 <- reshape_estimates(fit2, "cars_nocon", "c", car_vars)
t2 <- reshape_estimates(fit2, "trucks_nocon", "d", truck_vars)
t3 <- reshape_estimates(fit4, "cars_wcon", "c", car_vars)
t4 <- reshape_estimates(fit4, "trucks_wcon", "d", truck_vars)

table_85 <- merge(t1, t2, by = c("term", "stat"), all = TRUE)
table_85 <- merge(table_85, t3, by = c("term", "stat"), all = TRUE)
table_85 <- merge(table_85, t4, by = c("term", "stat"), all = TRUE)

table_85[, term := factor(term, levels = ordered_vars)]
setorder(table_85, term)
