...
table_85 <- reshape_estimates(fit2, "cars", "c", car_vars) %>%
  left_join(reshape_estimates(fit2, "trucks", "d", truck_vars)) %>%
  left_join(reshape_estimates(fit4, "cars", "c", car_vars),
    by = c("term", "stat")) %>%
  left_join(reshape_estimates(fit4, "trucks", "d", truck_vars),
    by = c("term", "stat"))

colnames(table_85) <- c("Variable", "Statistic", "Cars (No Con)", "Trucks (No Con)",
