...
if (!file.exists(fout)) {
  ...
  dist <- fread(paste0(dout, "/dist.csv"))
  setnames(dist, tolower(names(dist)))
  setorder(dist, c_e, c_i)

  trade_93 <- fread(paste0(dout, "/trade_93.csv"))
  setnames(trade_93, tolower(names(trade_93)))
  setorder(trade_93, c_e, c_i)

  trade_93 <- merge(trade_93, dist, by = c("c_e", "c_i"))
  rm(dist)

  gdp_ce_93 <- fread(paste0(dout, "/gdp_ce_93.csv"))
  setnames(gdp_ce_93, tolower(names(gdp_ce_93)))
  gdp_ce_93[, gdp_ce := gdp_ce * 0.775134]
  setorder(gdp_ce_93, c_e)

  gdp_ci_93 <- fread(paste0(dout, "/gdp_ci_93.csv"))
  setnames(gdp_ci_93, tolower(names(gdp_ci_93)))
  gdp_ci_93[, gdp_ci := gdp_ci * 0.775134]
  setorder(gdp_ci_93, c_i)

  trade_93 <- merge(trade_93, gdp_ce_93, by = "c_e")
  trade_93 <- merge(trade_93, gdp_ci_93, by = "c_i")
  trade_93 <- trade_93[vx != 0 & dist != 0]
  trade_93[, lnvx := log(vx)]
  trade_93[, lndist := log(dist)]
  trade_93[, lngdp_ce := log(gdp_ce)]
  trade_93[, lngdp_ci := log(gdp_ci)]
...
trade_93_2 <- trade_93[l_ex != 1 | l_im != 1]
trade_93_2[, d_ca := ifelse(l_ex == 2 & l_im == 2, 1, 0)]
...
trade_93_3 <- trade_93[l_ex != 2 | l_im != 2]
trade_93_3[, d_us := ifelse(l_ex == 1 & l_im == 1, 1, 0)]
...
trade_93_4 <- copy(trade_93)
trade_93_4[, d_ca := ifelse(l_ex == 2 & l_im == 2, 1, 0)]
trade_93_4[, d_us := ifelse(l_ex == 1 & l_im == 1, 1, 0)]
...
tab_c_e <- trade_93[, .(freq = .N), by = c_e]
tab_c_e[, percent := freq / sum(freq) * 100]
tab_c_e[, cum := cumsum(percent)]
tab_c_e

tab_c_i <- trade_93[, .(freq = .N), by = c_i]
tab_c_i[, percent := freq / sum(freq) * 100]
tab_c_i[, cum := cumsum(percent)]
tab_c_i

trade_93[, d_border := fifelse(
  (l_ex == 1 & l_im == 1) | (l_ex == 2 & l_im == 2), 0L, 1L
)]
trade_93[, lnvx := log(vx)]
trade_93[, lndist := log(dist)]
trade_93[, lngdp_ce := log(gdp_ce)]
trade_93[, lngdp_ci := log(gdp_ci)]
trade_93[, lnn_vx := lnvx - lngdp_ce - lngdp_ci]
