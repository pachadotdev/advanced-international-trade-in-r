...
if (!file.exists(fout)) {
  china_fdi <- as.data.table(read_dta(paste0(dout, "/china_fdi.dta")))
...
fit1 <- lm(formula_str, data = china_fdi[fiech3 > 0])

coef_dt <- as.data.table(summary(fit1)$coefficients, keep.rownames = "term")
...
china_fdi_8495 <- china_fdi[fiech3 > 0]
china_fdi_8895 <- china_fdi[fiech3 > 0 & year >= 1988]
china_fdi_9095 <- china_fdi[fiech3 > 0 & year >= 1990]
...
result <- rbindlist(Map(iv_summary_delta, models, periods))
kable(result, na = "", digits = 3)
```
