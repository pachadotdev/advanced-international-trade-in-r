...
# Number of country in the dataset
trefler[, c := max(indexc)]
trefler[, f := max(indexf)]
...
# Using the weight, convert all the data
trefler[, trat := at / (sigmaf * sqrt(sc))]
trefler[, athat2 := (v - sc * vfw) / weight]

setorder(trefler, country)

# Correlation, should be .28
cor(trefler[, .(trat, athat2)])

# Sign Test ----

trefler[, .(p = sum(trat * athat2 > 0) / .N), by = .(country, indexc)]

trefler[, .(p = sum(trat * athat2 > 0) / .N)]
...
trefler[, bw := bww / f]
trefler[, vfw := sum(v), by = indexf]
trefler[, vw := sum(delta * v), by = indexf]
...
# Using the weight, convert all the data
trefler[, trat := delta * at / weight]
trefler[, athat2 := (delta * v - sc * vw) / weight]
...
# Construct Aggregate Labor Endowment
trefler2 <- trefler[indexf %in% 7:9]
trefler2[, en := fcase(indexf == 7, 2L, indexf == 8, 3L, indexf == 9, 4L)]
trefler2 <- trefler2[, .(country, factor, at, v, en, indexc, sc)]

trefler3_labor <- trefler[!(indexf %in% 7:9),
...
trefler3 <- rbindlist(list(trefler2, trefler3_labor), fill = TRUE)
